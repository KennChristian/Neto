    ...
    for e in raw.get("entries", []):
        ...
        if answers:
            entries.append({"id": e.get("id", "?"), "patterns": pats,
                            ...
                            # verbatim scripted wording (event_* entries): the
                            # display shows this instead of the STT transcript
                            "ask": (e.get("ask") or [None])[0]})
...
def match(question: str):
    """Return {"id", "answer", "ask"} for a matched common question, else None."""
    try:
        if not enabled():
            return None
        ...
        for e in entries:
            ...
            if any(p.fullmatch(norm) for p in e["patterns"]):
                return {"id": e["id"], "answer": random.choice(e["answers"]), "ask": e.get("ask")}
...
def _event_flexible(norm: str, entries):
    ...
    if best is not None and best_ratio >= EVENT_FUZZY_RATIO:
        ...
        return {"id": best["id"], "answer": random.choice(best["answers"]), "ask": best.get("ask")}
    return None
