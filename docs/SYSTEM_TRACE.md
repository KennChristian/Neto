...
- LED ring (XVF3800 `LED_EFFECT` / `LED_COLOR` via the DoA USB handle, `_SpeakerDoA.set_led`, `led_state()`): listen / think / talk / idle set in `Gestures.start`, mic-muted red polled ~1 Hz in `_wake_stream`; `/maintain` Mechanical card "LED ring" row tests effect ids 0–6; `CJ_LED=0` off, `CJ_LED_<STATE>="effect:RRGGBB"` overrides.
