...
| `CJ_TTS_BACKEND` (app/.env) | `elevenlabs` | `voice_io.tts_elevenlabs_wav` → `voice/speak.synthesize` |
...
1. Import-time side effects: `.env` loading (`cj_chat`), `voice_io` defaults, **mic device pick** (`sd.default.device = reachymini_audio_src_plug`), `_speaker_doa = _SpeakerDoA()`, `_SENT_OUT = _SentenceOut()`.
...
5. `gestures.neutral()`; `print("Ready.")`; **`prewarm_boot()`** — daemon thread imports `stream_speak`, `voice.speak` (~4 s) and loads the entity dictionary (~0.9 s) so the first answer pays nothing.
6. `--wake` → `wake_loop()`.
...
- `_synth`: `process_tts_sentence` → `voice_io.tts_elevenlabs_wav(text, speed, previous_text)` → wav path (**no ffmpeg**; the OpenAI fallback still transcodes its mp3).
...
### 3g TTS — `voice_io.tts_elevenlabs_wav` → `voice/speak.synthesize`
...
- `[trace]` line per turn; section banners in `cj_voice_cloud.py`.
- Dashboard: HTTP/1.1 keep-alive, 500 ms state poll, fire-and-forget actions.
- Sentence tempo smoothing (`app/tempo_smooth.py`, called from `SentenceSpeaker._synth`): rate from the word alignment → WSOLA stretch ≤6 % toward a session-wide EMA (`/dev/shm/cj_tempo_avg.json`); opener never stretched; `[tempo]` log lines. ElevenLabs request stitching via `previous_request_ids` (voice/speak.py → voice_io → SentenceSpeaker._rids).
