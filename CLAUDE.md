...
|---|---|---|
| [`app/`](app/) | The runnable conversation app — `answer_pipeline.py` (CLI + pipeline functions), `dashboard.py` (Streamlit UI), requirements, voice/Piper local assets. Reads corpus from `../corpus/voice/` and `../corpus/{type}/{theme}/`. | [app/MANIFEST.md](app/MANIFEST.md) |
...
| [`tests/`](tests/) | pytest suite (`app/.venv/bin/python -m pytest tests/`): answer gate, canned answers, dashboard UI, and `test_floor_lease.py` for the one-mic invariant. | — |
...
- **Tests exist since 2026-08 under `tests/`** (pytest installed in `app/.venv` on 2026-09-10); the earlier statement below is kept for history.
