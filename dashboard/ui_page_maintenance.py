...
.tabs{flex-basis:100%;display:flex;gap:6px;flex-wrap:wrap}
.tabs button{padding:8px 14px;min-height:0;font-size:14px;font-weight:600;border-radius:999px}
.tabs button.on{background:var(--gold);color:#0d1117;border-color:var(--gold)}
[hidden]{display:none!important}
.sw{position:relative;display:inline-flex;align-items:center;gap:10px;cursor:pointer;user-select:none;font-weight:600}
.sw input{position:absolute;opacity:0;width:0;height:0}
.sw i{width:52px;height:30px;border-radius:999px;background:var(--btnline);border:1px solid var(--btnline);position:relative;transition:background .15s;flex:none}
.sw i::after{content:'';position:absolute;top:3px;left:3px;width:22px;height:22px;border-radius:50%;background:#fff;transition:left .15s}
.sw input:checked+i{background:var(--ok);border-color:var(--ok)}
.sw input:checked+i::after{left:25px}
.sw input:focus-visible+i{outline:2px solid var(--gold)}
...
/* robot mode banner (2026-09-02, user: "add on top of the maintenance ui the
   idle, listening, speaking mode") — same state rules as the /audience pill */
.mode{display:flex;flex-wrap:wrap;align-items:center;gap:8px 16px;margin:8px 0 0;padding:12px 16px;border:1px solid var(--line);border-radius:12px;background:var(--panel);transition:border-color .3s}
.mode .dot{width:18px;height:18px;border-radius:50%;background:#7a7368;flex:none;box-shadow:0 0 0 3px rgba(255,255,255,.06);transition:background .3s,box-shadow .3s}
.mode b.lbl{font-size:20px;letter-spacing:.14em;font-weight:700}
.mode .mdet{color:var(--dim);font-size:14px;margin-left:8px}
.mode .mage{color:var(--dim);font-size:13px;font-variant-numeric:tabular-nums}
.mode.listening{border-color:rgba(63,185,80,.6)}.mode.listening .dot{background:#3fb950;box-shadow:0 0 12px 2px rgba(63,185,80,.55);animation:mpulse 1.2s ease-in-out infinite}
.mode.thinking{border-color:rgba(201,169,97,.6)}.mode.thinking .dot{background:var(--gold);box-shadow:0 0 10px 1px rgba(201,169,97,.5);animation:mpulse 1.2s ease-in-out infinite}
.mode.speaking{border-color:rgba(229,72,77,.6)}.mode.speaking .dot{background:#e5484d;box-shadow:0 0 12px 2px rgba(229,72,77,.55);animation:mpulse .6s ease-in-out infinite}
.mode.muted .dot,.mode.down .dot{background:#e5484d}.mode.down{border-color:#e5484d}.mode.muted{border-color:rgba(229,72,77,.6)}
.msteps{display:flex;gap:6px;flex-wrap:wrap;margin-left:auto}
.msteps span{padding:3px 9px;border-radius:999px;border:1px solid var(--line);font-size:12px;color:var(--dim);white-space:nowrap}
.msteps span.active{border-color:var(--gold);color:var(--gold)}.msteps span.done{border-color:var(--ok);color:var(--ok)}.msteps span.failed{border-color:var(--bad);color:var(--bad)}
.mtags{flex-basis:100%;display:flex;gap:4px 14px;flex-wrap:wrap;font-size:13px;color:var(--dim)}
.mtags .on{color:var(--ok)}.mtags .warn{color:var(--gold)}.mtags .bad{color:#e5484d}
...
/* avatar-card state chips: same vocabulary as the /audience pill */
.chip.st-speaking{color:#e5484d}.chip.st-listening{color:var(--ok)}
.chip.st-thinking{color:var(--gold)}.chip.st-muted{color:#e5484d}
.chip.st-down{color:var(--bad)}.chip.st-idle{color:var(--dim)}
...
.mbar{display:inline-block;width:56px;height:8px;background:#21262d;border-radius:4px;vertical-align:middle;overflow:hidden;margin-right:10px}.mbar i{display:block;height:100%;background:#58a6ff}
...
<div class="card c6" data-tab="system"><h2>System</h2><div id="services"></div><div id="sys" class="dim">loading&hellip;</div></div>
...
<div class="stack" data-tab="audio">
...
<div class="card c8" data-tab="robot"><h2>Mechanical actions <span class="dim">(head, antennas, motors — runs via the voice app when idle)</span></h2>
...
<div class="card c8" data-tab="audio"><h2>Manual actions</h2>
  ...
  <div class="btns"><span class="lbl">Voice isolator</span>
  ...
  <div class="btns"><span class="lbl">Tools</span>
    <button onclick="audioConsole()">&#127911; Audio console</button>
...
<div class="card c8" data-tab="live"><h2>Sound <span class="dim">(which speaker, how loud)</span></h2>
  <div class="btns"><span class="lbl">Speaker</span>
    ...
    <button onclick="act('audio-both')" title="every answer on the Bluetooth speaker AND the laptop at once">&#128266;+&#128187; Speaker + laptop</button>
    ...
    <span class="dim">the watchdog puts audio back on a connected Bluetooth speaker within 15 s — switch it off first to stay on Internal; a working Laptop / Speaker + laptop / USB DAC choice is left alone</span></div>
  <div class="btns" id="vol-row"><span class="lbl">Volume</span>
    <button onclick="setVol(0)" title="silence (mute the output, keep listening)">&#128263; 0</button>
    <button onclick="setVol(Math.max(0,+$('vol').value-10))">&minus;</button>
    ...
    <button onclick="setVol(Math.min(100,+$('vol').value+10))">+</button>
    <button onclick="setVol(40)">40</button>
    <button onclick="setVol(70)">70</button>
    <button onclick="setVol(100)">100</button>
    <span class="dim" id="vol-hint">applies to the internal speaker and to any connected Bluetooth speaker (scales the stream below the speaker's own button level, so 100 = the speaker's setting); remembered across route switches, reconnects and reboots</span></div>
  <span id="msg4" class="dim"></span>
</div>
<div class="card c12" id="mic-card" data-tab="audio"><h2>Microphone meter <span class="dim" id="micstate">off</span></h2>
  <div class="btns"><span class="lbl">Meter</span>
    ...
    <span id="mp-ext"></span>
    <span class="dim" id="mic-hint">L = the filtered beam CJ actually hears (AEC + beamforming + noise suppression); R = your pick straight from the XVF3800: a raw mic as the DSP receives it, or the AEC reference (what the chip cancels against — flat while a Bluetooth speaker plays = it cannot hear itself to cancel). Runs only while this page is open; channel R goes back to the processed beam when off. A plugged-in USB/webcam microphone shows up as its own button and meters directly (chip channel R stays on the beam while it is picked).</span></div>
...
<div class="card" data-tab="live"><h2>Camera <span class="dim" id="camstate"></span></h2>
  ...
  <div class="btns"><span class="lbl">Focus</span>
    ...
    <b id="focusv" style="min-width:64px;display:inline-block">&hellip;</b>
    <span class="dim">fixed distances hold rock-steady on stage (autofocus prefers the nearest object in view — table, keyboard, the robot’s own housing); the slider sets any distance from 12 cm to infinity; Refocus returns to autofocus. Every change restarts the capture (~2 s)</span></div>
  <div class="btns"><span class="lbl">Size</span>
    ...
    <button class="sm" id="ce-hw" onclick="ctl('camera-enc-hw')" title="v4l2jpegenc — ~9% CPU but bigger frames">hw</button>
    <span class="dim">every change restarts the capture (~2 s); big sizes + sw encoder cost CPU</span></div>
...
<div class="card" data-tab="robot"><h2>LiveAvatar page <span class="dim" id="avstate"></span></h2>
  ...
  <div class="btns"><span class="lbl">View</span>
    ...
    <button class="sm" id="avv-full" onclick="ctl('avatar-view-full')"
      title="edge to edge — the avatar fills the page, plaques float over it">full screen</button>
    <span class="dim">the avatar video is 16:9, so &ldquo;portrait&rdquo; crops about half its width</span></div>
  ...
  <div class="btns"><span class="lbl">Change</span>
    ...
    <button id="av-apply" onclick="avApply()">Apply</button></div>
  ...
  <span class="dim">Apply checks the id against LiveAvatar first &mdash; a typo is refused instead of leaving
    the exhibit with a face that will not start. It then writes assets/liveavatar.json and tells the
    /face-avatar page to drop the old portrait and fetch the new one; a session already on air is ended first.
    Leave the key blank to keep the stored one.</span>
  <a class="dim" href="/face?key=" id="avlink" target="_blank">open page &#8599;</a></div>
<div class="card" data-tab="robot"><h2>Video clips <span class="dim" id="vidmsg"></span></h2>
  ...
  <span class="dim">&#9654; plays full-screen on the /face-avatar page; sound goes where Sound points &mdash; &ldquo;robot speaker&rdquo; is whatever the Audio card is routed to right now (the clip is refused if that speaker is not connected). Sync: with sound on the robot the picture waits for the speaker to be ready, then starts after the route's latency (internal ~120 ms, Bluetooth ~320 ms) plus this offset &mdash; sound BEFORE picture: raise it; picture before sound: lower it (negative allowed)</span></div>
...
<div class="card c6" data-tab="system"><h2>Bluetooth <span class="dim" id="btnow"></span></h2>
  ...
  <div class="btns"><span class="lbl">Speaker</span>
    <button onclick="act('audio-internal')">&#129302; Internal</button>
    <button onclick="act('audio-dac')">&#127911; USB DAC</button>
    <button onclick="act('audio-sony')">&#128266; Sony</button>
    <button onclick="act('audio-marshall')">&#128266; Marshall</button>
    <button onclick="act('audio-laptop')">&#128187; Laptop</button>
    <button onclick="act('audio-both')">&#128266;+&#128187; Speaker + laptop</button></div>
...
<div class="card c8" data-tab="conv"><h2>Conversation (raw vs corrected)</h2>
  <table id="conv"><tr><th>who</th><th>text</th></tr></table></div>
<div class="card" data-tab="conv"><h2>NER corrections (P0)</h2>
  <table id="ner"><tr><th>heard</th><th>&rarr; canonical</th><th>class</th><th>conf</th></tr></table></div>
<div class="card c12" data-tab="conv"><h2>Recent turns (tracking)</h2>
...
<div class="card c6" data-tab="logs"><h2>Logs
...
<div class="card c6" data-tab="conv"><h2>Entity dictionary overlay <span class="dim">(saves live, no restart)</span></h2>
...
function note(t){$('msg').innerText=t;for(const k of ['msg2','msg3','msg4'])if($(k))$(k).innerText=t;}
...
const TABS=[['live','Live'],['audio','Audio'],['robot','Robot'],['conv','Conversation'],['system','System'],['logs','Logs'],['all','All']];
function showTab(t){try{localStorage.setItem('cjtab',t)}catch(e){}
  ...
  document.querySelectorAll('.grid > [data-tab]').forEach(el=>{el.hidden=(t!=='all'&&el.dataset.tab!==t)});
  document.querySelectorAll('.grid > .sec').forEach(el=>{el.hidden=(t!=='all')});
  try{fitMeters()}catch(e){}}
(function(){const bar=$('tabs');if(!bar)return;TABS.forEach(([k,l])=>{const b=document.createElement('button');b.type='button';b.setAttribute('role','tab');b.dataset.t=k;b.textContent=l;b.onclick=()=>showTab(k);bar.appendChild(b)});
...
setInterval(camTick,2000);camTick();loadOv();loadLogs();vidList();vidSnd(VID_SND);
